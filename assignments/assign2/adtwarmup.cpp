...
STUDENT_TEST("duplicateNegatives, one negative input ") {
    Queue<int> q = {-3};
    Queue<int> expected = {-3, -3};

    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}

STUDENT_TEST("duplicateNegatives, input starts with negative elements ") {
    Queue<int> q = {-3, -5, 1, 7};
    Queue<int> expected = {-3, -3, -5, -5, 1, 7};

    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}

STUDENT_TEST("duplicateNegatives, input ends with negative elements ") {
    Queue<int> q = {1, 7, -3, -5};
    Queue<int> expected = {1, 7, -3, -3, -5, -5};
